...
class Idle:
    ...
    @staticmethod
    def draw(boy):
        boy.image.clip_draw(int(boy.frame) * 100, boy.action * 100, 100, 100, int(boy.x), int(boy.y))
    ...
    def draw(boy):
        if boy.face_dir == 1:
            boy.image.clip_composite_draw(int(boy.frame) * 100, 300, 100, 100,
                                        3.141592 / 2, '', int(boy.x - 25), int(boy.y - 25), 100, 100)
        else:
...
class Boy:
    def __init__(self):
        ...
        self.face_dir = 1
        self.dir = 0
        self.frame = 0.0  # 부드러운 애니메이션을 위해 float으로 변경
        self.action = 3
